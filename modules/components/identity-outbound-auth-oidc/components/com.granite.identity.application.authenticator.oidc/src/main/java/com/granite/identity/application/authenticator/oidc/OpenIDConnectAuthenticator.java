...
public class OpenIDConnectAuthenticator extends AbstractApplicationAuthenticator implements
    ...
    protected void processAuthenticationResponse(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            Map<ClaimMapping, String> subjectAttributes = getSubjectAttributes(oAuthResponse, authenticatorProperties);
            ...
            if (StringUtils.isNotBlank(idToken)) {
                ...
                String authenticatedUser = getUserNameFromSubjectAttributes(subjectAttributes);
                if (authenticatedUser.contains("@")) {
                    tenantDomain = MultitenantUtils.getTenantDomain(authenticatedUser);
                    authenticatedUser = MultitenantUtils.getTenantAwareUsername(authenticatedUser).replace(tenantDomain, "");
                }
                log.debug("For the federated user, identifier: " + authenticatedUser + " and domain:" +
                                  tenantDomain + " are used as authenticatedUser and tenantDomain");
                ...
                jsonObject.values().addAll(subjectAttributes.entrySet());
            ...
            claims.putAll(subjectAttributes);
            authenticatedUserObj.setUserAttributes(claims);

            context.setSubject(authenticatedUserObj);
            if (log.isDebugEnabled()) {
                for (Map.Entry<ClaimMapping, String> claim : claims.entrySet()) {
                    ClaimMapping claimMapping = claim.getKey();
                    log.debug("claim:" + claimMapping.getRemoteClaim().getClaimUri() + "='" + claim.getValue() + "'");
                }
            }
    ...
    private String getUserNameFromSubjectAttributes(Map<ClaimMapping, String> subjectAttrs)
            throws AuthenticationFailedException {
        for(Map.Entry<ClaimMapping, String> entry: subjectAttrs.entrySet()){
            String subj = entry.getKey().getRemoteClaim().getClaimUri();
            if(subj.equals(OIDCAuthenticatorConstants.Claim.NAME)){
                return entry.getValue();
            }
        }
        throw new AuthenticationFailedException("Cannot find federated User Identifier in the subject claims `name`");
    }
